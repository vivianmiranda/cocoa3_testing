    ...
    type, extends (TCAMBParameters) :: CAMBparams
        ...
        integer   :: which_potential = 1         ! DHFS: potential choice
        real(dl)  :: potentialparameter1 = 0._dl ! DHFS: first potential parameter
        real(dl)  :: potentialparameter2 = 0._dl ! DHFS: second potential parameter
        logical   :: want_binary_search  = .true.     ! DHFS: deciding if want or not binary search (default true)
    ...
    ! DHFS: Begins- validating my parameter
    if (this%which_potential < 0 .or. this%which_potential > 100) then
        OK = .false.
        write(*,*) '  Warning: potential type does not exist.', this%which_potential
    end if

    if (this%potentialparameter1<0._dl .or. this%potentialparameter1>100._dl) then
        OK = .false.
        write(*,*) '  Warning: strange potential parameter.', this%potentialparameter1
    end if

    if (this%potentialparameter2<0._dl .or. this%potentialparameter2>100._dl) then
        OK = .false.
        write(*,*) '  Warning: strange potential parameter.', this%potentialparameter2
    end if
