    ...
    else if (DarkEnergyModel == 'LATEQUINTESSENCE') then !DHFS: changing the name from EARLYQUINTESSENCE to LATEQUINTESSENCE 
        allocate (TLateQuintessence::P%DarkEnergy) ! DHFS: changing the name from TEalyQuintessence to TLateQuintessence
    else
    ...
    P%which_potential = Ini%Read_Int('which_potential')            !DHFS: read potential type from inifile
    P%potentialparameter1 = Ini%Read_Double('potentialparameter1') !DHFS: read potential parameter 1
    P%potentialparameter2 = Ini%Read_Double('potentialparameter2') !DHFS: read potential parameter 2  
    P%want_binary_search = Ini%Read_Logical('want_binary_search')  !DHFS: read binary search decision
