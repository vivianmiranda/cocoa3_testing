    ...
    procedure :: grho_cdm, grho_baryon, grho_photon, grho_masslessNu, grho_massiveNu ! DHFS
...
10      if (this%CP%Omnuh2 < 1.e-7_dl) this%CP%Omnuh2 = 0
        ...
        this%Omega_de = 1 -(this%CP%omch2 + this%CP%ombh2 + this%CP%omnuh2)/h2 - this%CP%omk  &
            - (this%grhornomass + this%grhog)/this%grhocrit !DHFS: 1 - (all other components)
    ...
    !DHFS- Begins grho for other components
    !DHFS- I'm mimicking the function grho_no_de but now for each component
    function grho_cdm(this,a) result(grhoa2)
    ...
    function grho_baryon(this,a) result(grhoa2)
    !DHFS: return 8*pi*G*rho_baryon*a**4 where rho_baryon includes only baryons 
        class(CAMBdata) :: this
        real(dl), intent(in)  :: a 
        real(dl) grhoa2

        grhoa2 = this%grhob * a
    end function grho_baryon

    function grho_photon(this,a) result(grhoa2)
    !DHFS: return 8*pi*G*rho_photon*a**4 where rho_photon includes only photons
        class(CAMBdata) :: this
        real(dl), intent(in) :: a
        real(dl) grhoa2

        grhoa2 =  this%grhog 
    end function grho_photon

    function grho_masslessNu(this, a) result(grhoa2)
    !DHFS Return 8*pi*G*rho_masslessNu*a**4
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) grhoa2

    grhoa2 =  this%grhornomass 
    end function grho_masslessNu

    function grho_massiveNu(this, a) result(grhoa2)
    !DHFS Return 8*pi*G*rho_massiveNu*a**4
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) grhoa2, rhonu
    integer nu_i

    grhoa2 =  0 !DHFS: starting with zero, we get only the contriubtion for massive \nu's
    ...
    end function grho_massiveNu
    !DHFS- Ends grho for other components
